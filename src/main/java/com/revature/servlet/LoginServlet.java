...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (user.getRtId() == 1) // If user is a employee
			request.getRequestDispatcher("emp_home.html").forward(request, response);  // path will be changed later
		else if (user.getRtId() == 2) //If user is an manager
